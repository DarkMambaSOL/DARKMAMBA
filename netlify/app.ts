...
// Enable CORS for external static hosts (e.g. Netlify, Vercel, etc.)
...
app.get("/api/download-netlify-zip", (req, res) => {
  try {
    ...
    if (indexHtmlContent && indexHtmlContent.includes("</head>")) {
      const selfUrl = (process.env.APP_URL || `${req.protocol}://${req.get("host")}`).replace(/\/$/, "");
      const scriptInjected = `
<script>
  try {
    localStorage.setItem("dmsol_custom_api_base", "${selfUrl}");
